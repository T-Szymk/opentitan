...
## Clock Signal
## clkgen_xil_ultrascale's MMCM is configured for a 90MHz reference input.
create_clock -add -name sys_clk_pin -period 11.111 -waveform {0 5.555} [get_ports IO_CLK]

## Explicit MMCM output clocks (clkgen_xil_ultrascale.sv, vcu118 params:
## DIVCLK_DIVIDE=3, CLKFBOUT_MULT_F=40.0 -> 90MHz*40/3 = 1200MHz VCO;
## CLKOUT0/CLKOUT2_DIVIDE=50 -> 1200/50 = 24MHz; CLKOUT4_CASCADE=TRUE chains
## CLKOUT4_DIVIDE=40 with CLKOUT6_DIVIDE=120 -> 1200/(40*120) = 250kHz).
## -multiply_by/-divide_by below are relative to CLKIN1 (90MHz), so they fold
## in DIVCLK_DIVIDE too: 4/15 = (40/3)/50, 1/360 = (40/3)/4800 -- NOT the same
## 12/50, 12/4800 cw340 uses (that ratio is only correct for DIVCLK_DIVIDE=1).
## Declared explicitly via -source/-multiply_by/-divide_by rather than the
## bare create_generated_clock [get_pins ...] auto-derive-and-rename form --
## that form silently creates nothing (just a "could not find an
## automatically derived clock to rename" warning) unless sys_clk_pin above
## is already a declared clock by the time it's evaluated.
create_generated_clock -name clk_main -source [get_pins ${clkgen}/CLKIN1] \
    -multiply_by 4 -divide_by 15 [get_pins ${clkgen}/CLKOUT0]
create_generated_clock -name clk_io_pre -source [get_pins ${clkgen}/CLKIN1] \
    -multiply_by 4 -divide_by 15 [get_pins ${clkgen}/CLKOUT2]
create_generated_clock -name clk_aon -source [get_pins ${clkgen}/CLKIN1] \
    -multiply_by 1 -divide_by 360 [get_pins ${clkgen}/CLKOUT4]
...
## System Clock (IO_CLK)
## IO_CLK is wired to the VCU118's dedicated EMCCLK pin (AL20, bank 65),
## which is not a global-clock-capable (GC) pin -- feeding it straight into
## clkgen_xil_ultrascale's MMCM fails Vivado's placer (Place 30-681,
## "Sub-optimal placement for a global clock-capable IO pin and MMCM pair").
## Fixed by adding an explicit BUFG between the pad and the MMCM
## (chiplevel.sv.tpl's vcu118 branch, u_io_clk_bufg) -- Xilinx's own
## suggested workaround for this exact rule. That BUFG's input is in turn
## directly IOB-driven with no intervening logic, tripping the same
## rule_gclkio_bufg DRC as JTAG_TCK above; accepted for the same reason --
## the MMCM's COMPENSATION=ZHOLD mode is designed to absorb reference-clock
## insertion-delay variance, so a non-dedicated route into the BUFG (not the
## MMCM's downstream clock tree, which remains fully dedicated) is an
## established, low-risk pattern for a board clock that isn't on a GC pin.
## A true GC differential source (e.g. VCU118's CLK_125MHZ, confirmed GC via
## its AY23/AY24 site names) would avoid this entirely, but requires adding
## differential pad support to OpenTitan's padring framework first -- see
## the TODO block below. NOTE: gen_dio_pads[79] is tied to the current
## padring generate-block ordering for IO_CLK's dedicated pad and may need
## re-deriving from a fresh synthesis run if that ordering ever changes.
set_property CLOCK_DEDICATED_ROUTE FALSE \
    [get_nets u_padring/gen_dio_pads[79].u_dio_pad/gen_input_only.u_ibuf/O]
...
##   beyond the PACKAGE_PIN/IOSTANDARD assignments in pins_darjeeling_vcu118.xdc.
## - IO_CLK: worked around via CLOCK_DEDICATED_ROUTE FALSE (see above) since
##   VCU118's EMCCLK pin isn't global-clock-capable. VCU118's CLK_125MHZ
##   (AY23/AY24) IS a confirmed GC differential pair (site names contain
##   _GC_) and would let this override be dropped entirely, but OpenTitan's
##   padring framework has no differential pad type today -- every layer
##   (hjson pad schema, chiplevel.sv.tpl's port-emission loop, padring.sv's
##   one-bit-per-pad array, prim_pad_wrapper.sv in all tech variants) assumes
##   one physical pin per logical pad. Revisit only if clean/jitter-free
##   clocking becomes a real requirement (e.g. SCA-sensitive work) -- not
##   worth the cross-cutting framework change otherwise.
## ============================================================
