...
% if target["name"] in ["cw340", "vcu118"]:
  ,
...
% if target["name"] == "verilator":
  ...
  assign vcc_supp = cnt < 4'h4 ? 1'b0 :
                    cnt < 4'h8 ? 1'b1 :
                    cnt < 4'hc ? 1'b0 : 1'b1;
% elif target["name"] in ["cw340", "vcu118"]:

  // Clock generation for cw340 (ChipWhisperer CW340, Kintex UltraScale
  // KU095) / vcu118 (AMD/Xilinx VCU118, Virtex UltraScale+ XCVU9P). AST's
  // own PLL is not synthesizable on an FPGA, so clkgen_xil_ultrascale
  ...
  // with -verilog_define AST_BYPASS_CLK). The MMCM's reference-clock
  // period/dividers are parameterized per target below -- cw340's 100MHz
...
<%
  clkin_period_ns = {"cw340": 10.000, "vcu118": 11.111}[target["name"]]
  divclk_divide   = {"cw340": 1,      "vcu118": 3}[target["name"]]
  clkfbout_mult_f = {"cw340": 12.000, "vcu118": 40.000}[target["name"]]
%>\
...
% endif
  ...
  clkgen_xil_ultrascale #(
    .AddClkBuf(0),
    .ClkInPeriod(${clkin_period_ns}),
    .DivClkDivide(${divclk_divide}),
    .ClkFbOutMultF(${clkfbout_mult_f})
  ) u_clkgen (
% if target["name"] == "vcu118":
    .clk_i(io_clk_buf),
% else:
...
% if target["name"] in ["verilator", "cw340", "vcu118"]:
...
% if target["name"] in ["asic", "cw340", "vcu118"]:
...
% if target["name"] in ["cw340", "vcu118"]:
    ...
    // timing closure is complete. Left at RTL defaults for v1.
